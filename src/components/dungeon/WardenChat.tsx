...
export function WardenChat({
  ...
  return (
    ...
    >
      ...
      <div className="flex max-h-64 flex-col gap-3 overflow-y-auto p-4 sm:max-h-72 [-webkit-overflow-scrolling:touch]">
        {messages.map((msg, i) => (
          <div
            key={`${msg.role}-${i}`}
      ...
      >
        <input
          ...
          className={`min-h-11 flex-1 rounded-xl border border-stone-700 bg-stone-900/80 px-3 py-2 text-base text-foreground outline-none sm:min-h-0 sm:text-sm ${
        ...
        <button
          ...
          className={`shrink-0 rounded-full px-5 py-2.5 text-sm font-semibold text-stone-950 transition disabled:opacity-50 sm:px-4 sm:py-2 ${
            salt
